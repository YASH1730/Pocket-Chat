...
exports.loginPOST = (req, res) => {
  ...
  if (!req.body.password || !req.body.email) {
    ...
    user.findOne({ Email: str }).then((data) => {
      if (data == null) {
        res.render("login.pug", { err: " User Not Found", title: "Log-In" });
      } else {
...
exports.profileDelete = (req, res) => {
  ...
  user.deleteOne({ UserName: name }).then((data) => {
    console.log(data.deletedCount);
  });

  feed.deleteOne({ UserName: name }).then((data) => {
    console.log(data.deletedCount);
  });
  ...
  follow.deleteMany({ Follower: name }).then((data) => {
    console.log(data.deletedCount);
  });

  follow.deleteMany({ Leader: name }).then((data) => {
    console.log(data.deletedCount);
  });

  massage.deleteOne({ Sender: name }).then((data) => {
    console.log(data.deletedCount);
  });

  massage.deleteOne({ Reciver: name }).then((data) => {
    console.log(data.deletedCount);
  });
